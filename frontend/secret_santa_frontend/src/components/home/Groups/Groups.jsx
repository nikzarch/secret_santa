...
export default function Groups() {
    ...
    const handleDeleteGroup = async (name) => {
        ...
        try {
            const res = await fetch(`http://localhost:8080/api/v1/groups/${name}`, {
                method: "DELETE",
                headers: { Authorization: `Bearer ${token}` },
            });
            if (!res.ok) {
    ...
    const fetchGroupUsers = async (group) => {
        ...
        try {
            const res = await fetch(`http://localhost:8080/api/v1/groups/${group.id}/participants`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            if (!res.ok) {
                const err = await res.json();
    ...
    const handleAddUser = async () => {
        ...
        try {
            const res = await fetch("http://localhost:8080/api/v1/groups/invite", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
    ...
    const handleKickUser = async (username) => {
        ...
        try {
            const res = await fetch("http://localhost:8080/api/v1/groups/kick-user", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
    ...
    const handleTransferOwner = async (username) => {
        try {
            const res = await fetch("http://localhost:8080/api/v1/groups/transfer-owner", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
