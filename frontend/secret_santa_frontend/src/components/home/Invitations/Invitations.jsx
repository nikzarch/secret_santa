...
export default function Invitations() {
    ...
    const fetchInvites = async () => {
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/groups/invites`, {
    ...
    const accept = async (id) => {
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/groups/invites/${id}/accept`, {
                method: "POST",
                headers: { Authorization: `Bearer ${token}` }
            });
            if (!res.ok) {
    ...
    const decline = async (id) => {
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/groups/invites/${id}/decline`, {
                method: "POST",
                headers: { Authorization: `Bearer ${token}` }
            });
