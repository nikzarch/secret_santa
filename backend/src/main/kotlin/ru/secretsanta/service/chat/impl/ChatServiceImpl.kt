...
class ChatServiceImpl (
    ...
    override fun sendMessageFromUserToUser(userFromId: Long, userToId: Long, eventId: Long, message: String): Unit {
        ...
        val hasAssignment = userFrom.sentAssignments.any { it.event.id == event.id && it.receiver.id == userTo.id }
    ...
    override fun getMessagesWithReceiver(currentUserId: Long, eventId: Long, pageable: Pageable): List<ChatMessageResponse> {
        ...
        val receiver = userRepository.findById(eventService.getReceiverForUser(eventId, currentUser.id).id).orElseThrow { NotFoundException("user not found") }
    ...
    override fun getMessagesWithSanta(currentUserId: Long, eventId: Long, pageable: Pageable): List<ChatMessageResponse> {
        val currentUser = userRepository.findById(currentUserId).orElseThrow { NotFoundException("user not found") }
        val event = eventRepository.findById(eventId).orElseThrow { NotFoundException("event not found") }
        val santa = userRepository.findById(eventService.getSantaForUser(eventId, currentUser.id).id).orElseThrow { NotFoundException("user not found") }
